...
def train(args):
    ...
    ########## Tworzenie shedulera szumu #############
    scheduler = LinearNoiseScheduler(num_timesteps=diffusion_config['num_timesteps'],
                                     beta_start=diffusion_config['beta_start'],
                                     beta_end=diffusion_config['beta_end'])
    ###############################################
    
    # Inicjalizacja warunkowych komponentów
    text_tokenizer = None
    text_model = None
    ...
    vae = None
    # Ładowanie VAE jeżeli brakuje latent
    ...
    # Ładowanie VAE i zatrzymanie parametrów jeżeli latent nie jest zapisany
    if not im_dataset.use_latents:
    ...
    for epoch_idx in range(num_epochs):
        ...
        for data in tqdm(data_loader):
            ...
            # Próbka losowego szumu
            noise = torch.randn_like(im).to(device)
            
            # Próbka timestep
            t = torch.randint(0, diffusion_config['num_timesteps'], (im.shape[0],)).to(device)
            
            # Dodawanie szumu zależne od timestep
